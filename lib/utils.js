...
export const CustomFile =
  isBrowser && typeof window.File !== 'undefined' && File;
export const CustomFileReader =
  isBrowser && typeof window.FileReader !== 'undefined' && FileReader;
...
export async function isAutoOrientationInBrowser() {
  if (isAutoOrientationInBrowser.cachedResult !== undefined)
  ...
  const testImageFile = await getFilefromDataUrl(
    testImageURL,
    'test.jpg',
    Date.now()
  );
  ...
  const testImageFile2 = await canvasToFile(
    testImageCanvas,
    testImageFile.type,
    testImageFile.name,
